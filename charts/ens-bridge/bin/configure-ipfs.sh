...
ipfs config --json DNS.Resolvers '{"tfuel.": "https://{{.Values.coredns.name}}.{{ .Values.namespace }}.svc.cluster.local/dns-query"}'

# IPFS gateway configuration - eth.limo is probably superfluous
ipfs config --json Gateway '{ "PublicGateways": { "*.tfuel.com": { "UseSubdomains": true}, "*.eth": { "UseSubdomains": true}}}'
